...
import { SFMDispatch, SFMThunk } from '../../types/redux.types';
...
export const thunkDispatchFileAction =
  (data: FileActionData<FileAction>): SFMThunk =>
  (_dispatch, getState) => {
    Logger.debug(`FILE ACTION DISPATCH: [${data.id}]`, 'data:', data);
    //@ts-ignore
    const { filemanager } = getState();
    const action = selectFileActionMap(filemanager)[data.id];
    const externalFileActionHandler = selectExternalFileActionHandler(filemanager);
    if (action) {
 ...
 * actions. When action is requested, SFM "prepares" the action data by extracting it
 * from Redux state. Once action data is ready, SFM executes some side effect and/or
 * dispatches the action to the external action handler.
 */
export const thunkRequestFileAction =
  <Action extends FileAction>(action: Action, payload: Action['__payloadType']): SFMThunk =>
  (dispatch, getState) => {
    Logger.debug(`FILE ACTION REQUEST: [${action.id}]`, 'action:', action, 'payload:', payload);
    //@ts-ignore
    const { filemanager } = getState();
    const instanceId = selectInstanceId(filemanager);

    if (!selectFileActionMap(filemanager)[action.id]) {
      Logger.warn(
    ...
    const selectedFiles = selectSelectedFiles(filemanager);
    ...
    const contextMenuTriggerFile = selectContextMenuTriggerFile(filemanager);
