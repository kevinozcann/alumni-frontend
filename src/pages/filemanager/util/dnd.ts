...
import { SFMDndDropResult, SFMDndFileEntryItem, SFMDndFileEntryType } from '../types/dnd.types';
...
export const useFileDrag = (file: Nullable<FileData>) => {
  ...
  const onDragStart = useCallback((): SFMDndFileEntryItem => {
    const item: SFMDndFileEntryItem = {
      type: SFMDndFileEntryType,
      payload: getDndStartPayload()
    };
    dispatch(thunkRequestFileAction(SFMActions.StartDragNDrop, item.payload));
    return item;
  }, [dispatch, getDndStartPayload]);
  const onDragEnd = useCallback(
    (item: SFMDndFileEntryItem, monitor: DragSourceMonitor) => {
      const dropResult = monitor.getDropResult() as SFMDndDropResult;
      if (
      ...
      dispatch(
        thunkRequestFileAction(SFMActions.EndDragNDrop, {
          ...item.payload,
  ...
  const item = useMemo<SFMDndFileEntryItem>(
    () => ({
      type: SFMDndFileEntryType,
...
export const useFileDrop = ({
  ...
  const onDrop = useCallback(
    (_item: SFMDndFileEntryItem, monitor) => {
      if (!monitor.canDrop()) return;
      const customDropResult: ExcludeKeys<SFMDndDropResult, 'dropEffect'> = {
        dropTarget: file
      };
  ...
  const canDrop = useCallback(
    (item: SFMDndFileEntryItem, monitor: DropTargetMonitor) => {
      if (
        forceDisableDrop ||
  ...
  const [{ isOver: dndIsOver, isOverCurrent: dndIsOverCurrent, canDrop: dndCanDrop }, drop] =
    useDropIfAvailable({
      accept: SFMDndFileEntryType,
      drop: onDrop,
      canDrop,
      collect
    });
