...
export type SFMTheme = typeof lightTheme;

export const darkThemeOverride: DeepPartial<SFMTheme> = {
  gridFileEntry: {
...
export const mobileThemeOverride: DeepPartial<SFMTheme> = {
  fontSizes: {
    rootPrimary: 13
  },
  margins: {
...
export const makeLocalSFMStyles = <C extends string = string>(
  styles: (theme: SFMTheme & MuiTheme) => any
  // @ts-ignore
): any => createUseStyles<SFMTheme, C>(styles);

export const makeGlobalSFMStyles = <C extends string = string>(
  makeStyles: (theme: SFMTheme & MuiTheme) => any
) => {
  const selectorMapping = {};
  const makeGlobalStyles = (theme: SFMTheme) => {
    const localStyles = makeStyles(theme as any);
    const globalStyles = {};
    const localSelectors = Object.keys(localStyles);
    localSelectors.map((localSelector) => {
      const globalSelector = `sFM-${localSelector}`;
  ...
  // @ts-ignore
  const useStyles = createUseStyles<SFMTheme, C>(makeGlobalStyles as any);
  return (...args: any[]): any => {
