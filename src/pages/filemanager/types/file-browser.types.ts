...
import { SFMIconProps } from './icons.types';
...
export interface FileBrowserHandle {
  ...
  /**
   * Method used to programatically trigger file actions in SFM.
...
export type SFMActionUnion = typeof SFMActions[keyof typeof SFMActions];
...
export interface FileBrowserProps {
  /**
   * An ID used to identify this particular SFM instance. Useful when there are
   * multiple SFM instances on the same page, and they need to interact with
   ...
   */
  onFileAction?: Nullable<GenericFileActionHandler<SFMActionUnion>>;
  ...
  /**
   * The flag that completely disables multiple file selection functionality. If disableSelection is true this won't take any effect.
   */
   ...
   * instance of SFM, while keeping other drag & drop functionality in tact.
   ...
   * be used to specify the default sort order in SFM.
   */
  defaultSortActionId?: Nullable<string>;

  /**
   * The ID of the file-view-setting action to activate by default. This field can
   * be used to specify the default file view in SFM.
   */
  defaultFileViewActionId?: string;

  /**
   * Determines whether the file selection should be cleared when user clicks
   * anywhere outside of SFM. By default, selection is cleared on outside click
   * unless the click target is a button.
   */
  clearSelectionOnOutsideClick?: boolean;

  /**
   * Icon component that will be used in this instance of SFM. Note that this
   * will only affect the current instance of SFM. If you wanna set the icon
   * component for all SFM instances, use the global config.
   */
  iconComponent?: ElementType<SFMIconProps>;
  ...
  /**
   * Configuration for the `react-intl` i18n library. SFM provides some default
