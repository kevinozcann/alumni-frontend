...
import {
  getFileData,
  getIsFileSelected,
  selectDisableMultipleSelection,
...
export const EssentialActions = {
    ...
    ({ payload, reduxDispatch, getReduxState }) => {
      if (payload.clickType === 'double') {
        if (FileHelper.isOpenable(payload.file)) {
          reduxDispatch(
            thunkRequestFileAction(SFMActions.OpenFiles, {
        ...
        const disableMultipleSelection = selectDisableMultipleSelection(getReduxState());
        if (FileHelper.isSelectable(payload.file) && !disableSelection) {
          if (payload.ctrlKey && !disableMultipleSelection) {
            // Multiple selection
            reduxDispatch(
          ...
          } else if (payload.shiftKey && !disableMultipleSelection) {
            // Range selection
  ...
  EndDragNDrop: defineFileAction(
    ...
    ({ payload, reduxDispatch, getReduxState }) => {
      ...
      reduxDispatch(
        thunkRequestFileAction(SFMActions.MoveFiles, {
          ...payload,
          files: droppedFiles
        })
      );
  ...
  OpenParentFolder: defineFileAction(
    {
      ...
      button: {
        ...
        toolbar: true,
        contextMenu: false,
        icon: SFMIconName.openParentFolder,
        iconOnly: true
      }
    } as const,
